...
# DBSCAN 실행
eps = 0.5  # 코사인 유사도 기준을 0.9 → 0.5로 완화
minPts = 3  # 최소 이웃 수를 5 → 3으로 완화
...
if len(set(valid_labels)) > 1:  # 클러스터가 2개 이상이어야 계산 가능
...
# 2D HTML 파일로 저장
fig_2d.write_html("clustering_result2_2d.html")
...
# 3D HTML 파일로 저장
fig_3d.write_html("clustering_result2_3d.html")